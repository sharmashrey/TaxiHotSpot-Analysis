...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val resDf = spark.sql("select joinResult.rectangle as jr, COUNT(*) from joinResult group by joinResult.rectangle order by joinResult.rectangle")
    resDf.createOrReplaceTempView("result")
    val ret = resDf.coalesce(1)

    return ret // YOU NEED TO CHANGE THIS PART
  }
